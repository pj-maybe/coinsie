...
def add_owner():
    ...
    r = _server_func.add_demo_owner(user_data["usertype"], user_data["username"], user_data["user_lcoin"], user_data["user_ccoin"])
    return r.text

@api.route('/show_data', methods=['GET'])
def show_data():
    print("called show_data!")
    r = _server_func.show_whole_data()
    return r

@api.route('/get_holdings', methods=['GET'])
def get_holdings():
    print("called get_holdings!")
    r = _server_func.get_holdings()    
    return r
...
def transfer_cap2():
    ...
    r = _server_func.transfer_cap2(user_data["username_from"], user_data["username_to"], user_data["shapename"], user_data["color"], user_data["amount"])    
    return r.text
